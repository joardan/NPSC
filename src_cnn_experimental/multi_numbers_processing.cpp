...
image_processor::image_processor(const std::string& file_path)
{
...
// Threshold the image
void image_processor::threshold_image()
{
...
void image_processor::find_contours()
{
    cv::findContours(image, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
}

// Find indices of likely number contours based on area
void image_processor::find_number_indices()
{
    double avgArea = 0;
    double sumArea = 0;
...
void image_processor::create_bounding_boxes()
{
    if (idx.empty()) return;
    int margin = 10;
...
void image_processor::crop_and_preprocess()
{
    int paddingSize = 2;
    int dilationSize = 2;
    ...
    for (int i = 0; i < processedImages.size(); ++i)
    {
        ...
        cv::Mat paddedImage(targetSize.height + 2 * paddingSize, targetSize.width + 2 * paddingSize, processedImages[i].type(), cv::Scalar(0));
        
        // Copy the processed image into the center of the padded image
        processedImages[i].copyTo(paddedImage(cv::Rect(paddingSize, paddingSize, processedImages[i].cols, processedImages[i].rows)));
        cv::dilate(paddedImage, paddedImage, kernel);
        cv::resize(paddedImage, paddedImage, targetSize);
        // Replace the original image with the padded one
        processedImages[i] = paddedImage;
    }
}

// Convert processed images to Eigen matrices
std::vector<std::vector<Eigen::MatrixXd*>> image_processor::convert_to_eigen()
{
...
void image_processor::display_image(const std::string& window_name, const cv::Mat& img)
{
...
void image_processor::process_demo()
{
...
void image_processor::process_demo_lite()
{
    display_image("Image", image);
    threshold_image();
    display_image("Image", image);
